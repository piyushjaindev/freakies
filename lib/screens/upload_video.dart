...
class _UploadVideoState extends State<UploadVideo> {
  ...
  createPostMethod() async {
    ...
    if (widget.currentUser.id == _firebaseUser.uid) {
//      await compressVideo();
//      await getThumbnail();
      setState(() {
        isProcessing = true;
      ...
      try {
        ...
        postRef
          ...
          'videoURL': videoURL,
          'timestamp': dateTime,
          'photoURl': widget.currentUser.dpURL,
          'tags': tags
        });
  ...
  uploadVideo() async {
    try {
      final uploadTask =
          storageRef.child('post_$randomID.mp4').putFile(outputFile);
  ...
  uploadThumbnail() async {
    try {
      final uploadTask = storageRef
          .child('post_${randomID}_thumbnail.jpg')
                  ...
                  content: Column(
                    children: <Widget>[
                      Container(
                          ...
                          child: _file == null
                              ? Text('Please go back and select a file first.')
                              : GestureDetector(
                                  onTap: () {
                        ...
                        children: <Widget>[
                          ...
                          Expanded(
                            child: RangeSlider(
                              ...
                              max: duration,
                              values:
                                  _values == null ? RangeValues(0, 0) : _values,
                            ),
                          ),
                          Text('${(duration ~/ 60)}:${(duration % 60).toInt()}')
                        ],
                      )
                    ],
                  )),
